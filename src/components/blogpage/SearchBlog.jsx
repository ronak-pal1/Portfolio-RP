...
const SearchedBlogBox = ({ blog }) => {
  return (
    ...
    >
      <p className="text-blue-500 text-xl font-poppins">{blog?.node.title}</p>
      <p className="line-clamp-2 text-sm font-light dark:text-white">
        {blog?.node.brief}
      </p>
    </Link>
  );
      ...
      <div className="w-1/2 relative">
        <div className="flex items-center">
          <input
            ...
            className="bg-slate-100 dark:bg-slate-800 py-4 px-4 font-poppins outline-none  rounded-l-md w-full placeholder:select-none dark:text-white"
          />
          <button
            onClick={() => searchBlogs(searchQuery)}
            className="bg-blue-600 py-4 px-4 text-white font-poppins rounded-r-md"
          >
            Search
          </button>
        </div>
        {searchedBlogs.length != 0 && (
          <div className="w-full h-96 bg-white dark:bg-black border border-slate-100 dark:border-none rounded-md absolute mt-3 p-5 shadow-md overflow-hidden">
            <p className="text-sm text-slate-500 font-poppins">
              Found {searchedBlogs.length} results
            </p>
            <div className="flex flex-col space-y-3 w-full h-full mt-7 overflow-y-scroll no-scrollbar">
